...
def view_accounts(data):
    ...
    for account in data:
        ...
        if not account.get("notes") == "User did not enter notes.":
...
def add_account(key, data):
    ...
    match password_prompt:
        case "1":
            password = gp.getpass('Enter password: ').strip()

            if password == '':
                print('Password cannot be empty.')
                return
                
            confirm_password = gp.getpass('Confirm password: ').strip()
    ...
    for account in data:
        if account.get("service").lower() == service:
            ...
            if action == "delete":
                ...
                print_modify_opts()
                modify_prompt = input("Enter choice: ").lower().strip()
