...
def get_key(mode=None):
    ...
    if mode == "change":
        master_pass = gp.getpass("New master password: ").strip().encode()
        confirm_master_pass = gp.getpass("Confirm new master password: ").strip().encode()
        if master_pass != confirm_master_pass:
            print("Passwords do not match.")
            return
    elif mode == "pre_change":
        master_pass = gp.getpass("Current master password: ").strip().encode()
    elif os.path.exists("./resources/database.json"):
        master_pass = gp.getpass("Master password: ").strip().encode()
    else:
        master_pass = gp.getpass("Set master password (first time setup): ").strip().encode()
        confirm_master_pass = gp.getpass("Confirm master password (first time setup): ").strip().encode()
